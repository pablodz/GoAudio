...
func generate(dur int, shape synth.Shape, ampStream, freqStream *breakpoint.BreakpointStream, wfmt wave.WaveFmt) []wave.Frame {
	...
	case synth.TRIANGLE:
		...
		lookupTable = triangleTable
	default:
		panic(fmt.Sprintf("Shape: %v not yet supported", shape))
	}

	osc, err := synth.NewLookupOscillator(wfmt.SampleRate, lookupTable, 0.0)
	if err != nil {
		panic(err)
	}
	...
	for i := range frames {
		...
		frames[i] = wave.Frame(amp * osc.InterpolateTick(freq))
	}
